...
namespace HoloToolkit.Unity.InputModule
{
    ...
    public class TapToPlace : MonoBehaviour, IInputClickHandler
    {
        ...
        [SerializeField] bool followParent = false;
        ...
        protected virtual void Update()
        {
            ...
            float xPos = x ? (followParent? transform.parent.position.x : transform.position.x) : placementPosition.x, yPos = y ? (followParent ? transform.parent.position.y : transform.position.y) : placementPosition.y, zPos = z ? (followParent ? transform.parent.position.z : transform.position.z) : placementPosition.z;
            // update the placement to match the user's gaze.
            interpolator.SetTargetPosition(new Vector3(xPos, yPos, zPos));

            // Rotate this object to face the user.
			interpolator.SetTargetRotation(Quaternion.Euler(0, Camera.main.transform.localEulerAngles.y + 180, 0));


			Vector3 normal = -Camera.main.transform.forward;     // Normally the normal is best set to be the opposite of the main camera's forward vector
